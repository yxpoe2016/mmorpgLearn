...
namespace Network
{
    ...
    public class MessageDispatch<T> : Singleton<MessageDispatch<T>>
    {
        ...
        public void Dispatch(T sender, SkillBridge.Message.NetMessageResponse message)
        {
            ...
            if (message.guildCreateReq != null) { MessageDistributer<T>.Instance.RaiseEvent(sender, message.guildCreateReq); }
            if (message.guildCreateRes != null) { MessageDistributer<T>.Instance.RaiseEvent(sender, message.guildCreateRes); }
